...
// router.get('/:id/subjects/:user_id', async (req, res) => {
//   const groupId = req.params.id;
//   const teacherId = req.params.user_id;  // Получаем ID преподавателя из JWT (или сессии)
...
//     // SQL запрос для получения предметов, которые ведет преподаватель для группы
//     // const query = `
...
//     // const [rows] = await db.query(query, [teacherId, groupId]);

//     if (isSeniorTeacher) {
//       // Если senior_teacher - получаем все активные предметы группы
//       query = `
//           SELECT s.subject_id, s.subject_name
//           FROM subjects s
//           JOIN course_subjects cs ON s.subject_id = cs.subject_id
...
//       params = [groupId];
//     } else {
//       // Обычный преподаватель - только свои предметы
//       query = `
//           SELECT s.subject_id, s.subject_name
//           FROM subjects s
//           JOIN course_subjects cs ON s.subject_id = cs.subject_id
...
// router.get('/:groupId/subject/:subjectId/grades', async (req, res) => {
...
// router.post('/:groupId/subject/:subjectId/grades', async (req, res) => {
//   const groupId = req.params.groupId;
//   const subjectId = req.params.subjectId;
//   const grades = req.body.grades;
//   const editorId = req.body.editor_id; // ID пользователя, который редактирует
...
// Получить студентов группы
router.get('/:id', groupController.getStudentsByGroupId);
// Получить предметы преподавателя для группы
router.get('/:id/subjects/:user_id', groupController.getSubjectsByGroupIdAndTeacherId);
// Получить оценки
router.get('/:groupId/subject/:subjectId/grades', groupController.getGradesByGroupIdAndSubjectId);
// Сохранить оценки
router.post('/:groupId/subject/:subjectId/grades', groupController.saveGrades);
